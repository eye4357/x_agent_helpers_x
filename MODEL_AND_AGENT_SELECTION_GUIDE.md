...
- Under Prompt B (5.5 escalation only), if the current work is deterministic and there is a safe downgrade path to GPT 5.3 Codex, stop escalation immediately and hand off with the last green commit SHA, CI run/job evidence, and the next smallest deterministic slice.
- When an adjacent deterministic family is fully closed and selecting the next family requires scope choice, treat that boundary as a handoff point under Prompt B unless there is a genuine high-complexity ambiguity that requires 5.5-level reasoning.
...
- After closing a DOCX sibling family, determine the next true no-assumption gap by computing live parity-index coverage from the test file; if DOCX metrics are complete, continue with one adjacent nested XLSX `sheets[0]` subkey parity assertion (start with `name`) before expanding to broader payload checks.
- Within nested XLSX `sheets[0]` parity closure, continue adjacent scalar subkeys in deterministic order (`name` then `path`) before list/map-heavy fields (`headers`, `cell_type_counts`, `freeze_pane`, `page_margins`).
...
- For deterministic XLSX nested `sheets[0]` parity slices, derive adjacency from `xlsx_sheet_summary` producer order and, if earlier slices skipped keys, continue with the first still-unpaired key before advancing (for the current backlog: `row_count`, then `column_count`, then `styled_cell_count`, then `cell_type_counts`).
...
- For DOCX `style_definition_ids` value-contract siblings, close comparisons in strict order (`1`, `2`, `3`) and mirror the same comparison-index bullet in changelog, release notes, and change-control within the same slice.
- When a DOCX metric is already present in the structure metric inventory and rendered Markdown rows but missing JSON comparison contracts (for example `table_cell_count`), treat it as the next no-assumption deterministic family start.